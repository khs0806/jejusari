...
<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=2992107a6cdd4a70cae5c448140c5fd1&libraries=services"></script>
...
<div class="container" id="sales_detail">
	<!-- 매물이름, 가격 -->
	<div id="title">
		<span>${salesDto.sales_title }</span> <span>${salesDto.sales_category_rent }</span>
	...
	<div id="info">
		<div class="borker">
			<img src="${root}/img/중개인.png" width="25px" height="25px" /> <span>${memberDto.member_name }
				( ${salesDto.member_id } )</span>
		</div>

		<div class="scrap_report">
			<img class="scrap_star" src="${root}/img/star1.png" width="20px"
				height="20px" /> <span class="scrap_count">${scrap_count }</span> <input
				type="button" name="scrap_btn" value="스크랩"
				class="btn btn-outline-warning btn-sm scrap_btn" /> <input
				type="button" name="report_btn" value="신고하기"
				class="btn btn-outline-danger btn-sm report_btn" />
		</div>
	</div>

	<!-- 주소, 매물요약 -->
	<div id="map_plus_sales" class="row justify-content-between">
		<div class="col mr-3 mt-3" id="map_detail"
			style="width: 50%; height: 250px; margin-top: 10px;"></div>
		<div class="col ml-2">
			<div class="input-group input-group  mt-3">
				...
				<input type="text" class="form-control" readonly
					value="${salesDto.sales_category_type }">
			</div>
			<div class="input-group input-group mt-3">
				<div class="input-group-prepend">
					<span class="input-group-text" id="inputGroup-sizing ">면 적
						(m²)</span>
				</div>
				<input type="text" class="form-control" readonly
					value="${salesDto.sales_area } m²">
			</div>
			<div class="input-group input-group  mt-3">
				<div class="input-group-prepend">
					<span class="input-group-text" id="inputGroup-sizing ">평 형</span>
				</div>
				<fmt:formatNumber var="pyung" value="${salesDto.sales_area/(3.3) }"
					type="number" pattern="0.0" />
				<input type="text" class="form-control" readonly value="${pyung} 평">
			</div>
			<div class="input-group input-group  mt-3">
				<div class="input-group-prepend">
					<span class="input-group-text" id="inputGroup-sizing ">중 공 년
						도</span>
				</div>
				<input type="text" class="form-control" readonly
					value="${salesDto.sales_build_year} 년도">
			</div>
			<div class="input-group input-group  mt-3">
				<div class="input-group-prepend">
	...
	<div id="sales_index">
		...
		<div class="all_index">
			...
			<div class="index_sum">
				<ul>
					<li><img class="factor_img" src="${root}/img/fruit.png"
	...
	<div class="input-group mb-3">
		<div class="form-control text-justify sales-content"
			aria-label="Sizing example input"
			aria-describedby="inputGroup-sizing-default">
			${salesDto.sales_content}</div>
	</div>
	...
	<div id="sales_img">
		<div class="img_all">
			<c:forEach var="salesImgDto" items="${salesImgDtoList}" end="2">
				<div class="img">
					<img src="${root }${salesImgDto.image_url}" width="310px"
						height="350px" />
				</div>
			</c:forEach>
			<c:if test="${fn:length(salesImgDtoList)>3}">
				<div class="img_more">
					<img src="${root }/img/next1.png" width="30px" height="30px" />
				</div>
				...
				<div class="img_small">
					<img src="${root }/img/next2.png" width="30px" height="30px" />
				</div>
			</c:if>
		</div>
	</div>
	...
	<!-- 중개인정보,매물더보기 -->
	<div id="member_btn">
		<input type="button" name="member_info" value="중개인 정보"
			class="btn btn-outline-info btn-sm" /> <input type="button"
			name="more_sales" value="매물 더보기" class="btn btn-secondary btn-sm" />
	</div>
	...
	<c:if test="${member_level=='admin'}">
		...
		<div id="plus_btn">
			<input type="button" name="update" value="신고처리"
				class="btn btn-light btn-sm"
				onclick="update('${root}','${salesDto.sales_number}')" />
			<!-- dark? -->
			<input type="button" name="delete" value="삭제"
				class="btn btn-light btn-sm"
				onclick="del('${root}','${salesDto.sales_number}')" />
		</div>
	</c:if>
</div>
